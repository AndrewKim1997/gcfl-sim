...
def test_u_orth_penalty_strictly_orthogonal_negative_delta():
    ...
    u = rng.normal(size=128); u -= u.mean()
    r = rng.normal(size=128); r -= r.mean()
    # make s orthogonal to u: remove projection
    proj = (np.dot(r, u) / np.dot(u, u)) * u
    s = r - proj
    m = -0.5  # negative monitoring → PoC > 0
    metrics = mech({}, u, s, m, rng, alpha=0.5, pi=0.0, phi=1.0, benign_threshold=0.0, neutralize_when_deltaU_ge_0=False)
    assert metrics["PoC"] > 0 and metrics["PoG"] == 0
    assert metrics["DeltaU"] < 0  # penalty applied
